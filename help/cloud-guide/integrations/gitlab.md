...
1. Envie os arquivos do projeto para o novo repositório do GitLab. Lembre-se de manter todos os nomes de ramificação iguais.
   ...
   Se você estiver começando com um novo repositório do GitLab, talvez seja necessário usar o `-f` , pois o repositório remoto não corresponde à sua cópia local.

1. Verifique se o repositório do GitLab contém todos os arquivos de projeto.

## Habilitar a integração do GitLab

Use o `magento-cloud integration` comando para habilitar a integração do GitLab e obter o URL de carga para o webhook do GitLab para enviar atualizações do GitLab para o projeto de infraestrutura do Adobe Commerce na nuvem.
...
| `--base-url` | URL do GitLab (`https://gitlab.com/` se o GitLab for usado na versão SaaS) |
| `--server-project` | Nome do projeto no GitLab (parte após o URL base) |
| `--build-merge-requests` | Um _opcional_ parâmetro que instrui o Adobe Commerce na infraestrutura em nuvem a criar um novo ambiente para cada solicitação de mesclagem (`true` por padrão) |
| `--merge-requests-clone-parent-data` | Um _opcional_ parâmetro que instrui o Adobe Commerce na infraestrutura em nuvem a clonar os dados do ambiente pai para solicitações de mesclagem (`true` por padrão) |
| `--fetch-branches` | Um _opcional_ parâmetro que faz com que o Adobe Commerce na infraestrutura em nuvem busque todas as ramificações do local remoto (como ambientes inativos) (`true` por padrão) |
| `--prune-branches` | Um _opcional_ parâmetro que instrui o Adobe Commerce na infraestrutura em nuvem a excluir ramificações que não existem no local remoto (`true` por padrão) |

>[!WARNING]
>
>A variável `magento-cloud integration` comando substitui _all_ no seu projeto Adobe Commerce on cloud infrastructure com o código do seu repositório GitLab. Isso inclui todas as ramificações, incluindo a `production` filial. Essa ação acontece instantaneamente e não pode ser desfeita. Como prática recomendada, é importante clonar todas as ramificações do projeto Adobe Commerce na infraestrutura em nuvem e enviá-las para o repositório do GitLab antes de adicionar a integração do GitLab.

**Para habilitar a integração com o GitLab**:
...
1. Copie o **URL de gancho** exibido pela saída de retorno.
...
Para comunicar eventos — como solicitações de push ou mesclagem — ao servidor Git da nuvem, você deve [criar um webhook](https://docs.gitlab.com/ee/user/project/integrations/webhooks.html#overview) para seu repositório GitLab

1. No repositório do GitLab, clique no link **Configurações** guia.

1. Na barra de navegação esquerda, clique em **Webhooks**.

1. No _Webhooks_ formulário, edite os seguintes campos:

   - **URL**: Insira o `Hook URL` retornado quando você ativou a integração do GitLab.
   - **Token secreto**: Insira um segredo de verificação, se necessário.
   - **Acionador**: Verificar `Merge request events` e/ou `Push events` dependendo das suas necessidades.
   - **Habilitar verificação SSL**: você deve selecionar essa opção.

1. Clique em **Adicionar webhook**.

### Testar a integração

Após configurar a integração do GitLab, é possível verificar se a integração está operacional usando o `magento-cloud` CLI:
...
Use o `magento-cloud` CLI `environment:push` para criar e ativar um novo ambiente. Consulte [Criar uma ramificação de nuvem](bitbucket.md#create-a-cloud-branch).

## Remover a integração

Use o `magento-cloud` CLI `integration:delete` para remover a integração. Consulte [Remover a integração](bitbucket.md#remove-the-integration).
